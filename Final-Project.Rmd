...
car_rec <- recipe(logDist ~ Side + Vis+ Temp+ Wind + Precip+ Weather_Condition + Crossing + Junction + Traffic_Signal + Sunrise_Sunset, data = accidents) %>%
...
mod1_output %>% 
  #augment() %>%
  ggplot(aes(x = Temp, y = resid)) + #note patterns in residual/error
  geom_point() +
  geom_smooth() +
  geom_hline(yintercept = 0) +
  theme_classic()
