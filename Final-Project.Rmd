...
accident_cv <- vfold_cv(accident_clean, v = 10) # this is the random part
...
lm_lasso_spec <- 
    linear_reg() %>%
    set_args(mixture = 1, penalty = tune()) %>% ## mixture = 1 indicates Lasso, we'll talk about penalty later
...
car_rec <- recipe(logDist ~ `Side` + `Visibility(mi)` + `Temperature(F)` + `Wind_Speed(mph)` + `Precipitation(in)` + `Weather_Condition` + `Crossing` + `Junction` + `Traffic_Signal` + `Sunrise_Sunset`, data = accident_sub) %>%
    step_nzv(all_predictors()) %>%
    step_other(all_nominal_predictors()) %>% 
    step_normalize(all_numeric_predictors()) %>%
    step_dummy(all_nominal_predictors()) #IS THIS RIGHT

model_wf <- workflow() %>%
    add_recipe(car_rec) %>%
    add_model(lm_spec)

fit_model <- model_wf %>% fit(data = accident_sub)
...
mod1_output <- fit_model %>% 
    predict(new_data = accident_sub) %>% #this function maintains the row order of the new_data
    bind_cols(accident_sub) %>%
